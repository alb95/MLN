...
t_hosvd = zeros(1,30);
t_rhosvd = zeros(1,30);
t_seq_rhosvd = zeros(1,30);
t_seq_mln = zeros(1,30);
t_mln = zeros(1,30);
...
r = [40,40,40];
    ...
    for i = 1:30
        N = [64+6*i,64+6*i,64+6*i];
        T = create_exponential_decaying_tensor(N, sigma);
        %tic
        %B_hosvd = multilinear_svd(T, [r,r,r]);
        %t_hosvd(i) = toc;
        t_rhosvd(i) = timeit(@() multilinear_hmt(T, [r,r,r]));
        t_mln(i) = timeit(@() multilinear_nystrom(T, [r,r,r], [floor(r/2),floor(r/2), floor(r/2)]));
        t_seq_rhosvd(i) = timeit(@() sequential_multilinear_hmt(T, [r,r,r]));
